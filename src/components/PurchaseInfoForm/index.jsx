...
function PurchaseInfoForm(props) {
    ...
    const validationSchema = Yup.object().shape({
        email: Yup.string().required('Vui lòng nhập email cc'),

        name: Yup.string().required('Vui lòng nhập họ tên cc'),
    ...
    const renderFields = (inputs, onChange) => {
        return inputs.map(input => {
            ...
            if (input.type === 'checkbox') {
                return (
                    <FormControlLabel key={'checkbox' + input.name}
                        control={<Checkbox
                            name={input.name}
                            onChange={onChange}
                            color="primary" />}
            ...
            if (input.type === 'textarea') {
                return (
                    <FastField key={'textarea' + input.name}
                        name={input.name}
            ...
            return (
                <FastField key={'input' + input.name}
                    name={input.name}
                    component={InputField}
